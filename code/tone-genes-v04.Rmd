---
title: 'Tone and genes: new data and methods support the negative effect of the "derived" allele of *ASPM* on tone, but not of *Microcephalin*'
...
This `Rmarkdown` script contains the full results supporting the main paper.
...
I will denote the "derived" alleles of *ASPM* and *MCHP1* (*Microcephalin*) as *ASPM*-D and *MCPH1*-D, respectively.
...
*Metapopulations* are fully included within the language families (and there are just a few with more than one sample), so I ignored them here as there is too little data to model them meaningfully.

However, the *language families* need to be modelled (even if most have only one or a few languages), as it is important to deal with the genealogical non-independence between languages: I thus model them as *random effects*.

*Macroareas*, on the other hand, need a special attention for several reasons:

- there is a very skewed geographical sampling in the data, with none from Australia and very few from the Americas, the dataset being dominated by Eurasia and Africa;
- the macroareas (as defined by Glottolog) have a very unclear status;
- the distribution of *ASPM*-D and and especially of *MCPH1*-D are highly skewed geographically, being almost absent from Africa.

As North and South America each have very few data points, I collapsed them both into "America".

Because of the above considerations, I treated "macroareas" as a *fixed effect*.

Moreover, the major split in the genetic data is between "Africa" and the rest of the world which, coupled with the fact that some methods cannot gracefully handle multi-valued factors, made me dichotomise (for some analyses) the *macroarea* in into "Africa" vs "the rest of the world".
...
For *tone1* I used (mixed-effects) *logistic regression* (as implemented by `glmer()`), and for the allele frequencies I used (mixed-effects) *beta regression* (as implemented by `glmmTMB()`, after replacing all $0.0$ values by $10^{-7}$ and all $1.0$ by $1.0-10^{-7}$, respectively).
...
I fitted the "full model" of a mixed-effects logistic regression where *tone1* is predicted by *macroarea*, *ASPM*-D and *MCPH1*-D (with their *interactions*) as fixed effects, and language *family* as random effect, and I found that:

- the "null model" (i.e., intercept and random effect only) has `r sprintf("ICC = %.1f%%", 100*performance::icc(m_tb_0)$ICC_adjusted)`[^ICC_interpretation], showing that *tone1* is strongly clustered within families;
- the *interactions* drop out: there does not seem to be an interaction between the two alleles (`r sprintf("*p* = %.2g", anova(m_tb_full_old, m_tb_1)[2,"Pr(>Chisq)"])`), nor between the alleles and macroarea  (`r sprintf("*p* = %.2g", anova(m_tb_full_old, m_tb_2)[2,"Pr(>Chisq)"])`);
...
- *ASPM*-D and *MCPH1*-D together predict *tone1* (`r sprintf("*p* = %.2g", anova(m_tb_0, m_tb_nom)[2,"Pr(>Chisq)"])`, `r sprintf("R^2^ = %.1f%%", 100*performance::r2(m_tb_nom)$R2_marginal)`): 
  ...
  + as does *MCPH1*-D by itself (`r sprintf("*β* = %.1f ± %.1f, *p* = %.2g", summary(m_tb_mcph1)$coefficients["MCPH1_freq_wavg", "Estimate"], summary(m_tb_mcph1)$coefficients["MCPH1_freq_wavg", "Std. Error"], anova(m_tb_0, m_tb_mcph1)[2,"Pr(>Chisq)"])`, `r sprintf("R^2^ = %.1f%%", 100*performance::r2(m_tb_mcph1)$R2_marginal)`);
- however, *macroarea*, *ASPM*-D and *MCPH1*-D seem to carry pretty much the same information about *tone1*, i.e., after including *macroarea*: *ASPM*: `r sprintf("*β* = %.1f ± %.1f, *p* = %.2g", summary(m_tb_aspm_m)$coefficients["ASPM_freq_wavg", "Estimate"], summary(m_tb_aspm_m)$coefficients["ASPM_freq_wavg", "Std. Error"], summary(m_tb_aspm_m)$coefficients["ASPM_freq_wavg", "Pr(>|z|)"])`; *MCPH1*: `r sprintf("*β* = %.1f ± %.1f, *p* = %.2g", summary(m_tb_mcph1_m)$coefficients["MCPH1_freq_wavg", "Estimate"], summary(m_tb_mcph1_m)$coefficients["MCPH1_freq_wavg", "Std. Error"], summary(m_tb_mcph1_m)$coefficients["MCPH1_freq_wavg", "Pr(>|z|)"])`.
...
To better understand this overlap between family, macroarea and the two "derived" alleles, I regressed (separately) the *ASPM*-D and *MCPH1*-D on the *macroarea*, using mixed-effects beta regression with language family as random effect:

- the distribution of the alleles is very strongly clustered within *families*: for *ASPM*: `r sprintf("ICC = %.1f%%", 100*performance::icc(m_aspm_macroarea)$ICC_adjusted)`, for *MCPH1*: `r sprintf("ICC = %.1f%%", 100*performance::icc(m_mcph1_macroarea)$ICC_adjusted)`;
...
- in fact, separating Africa vs the rest of the world seems to drive most of this effect (*ASPM*: `r sprintf("*p* = %.2g", anova(m_aspm_0, m_aspm_africa)[2,"Pr(>Chisq)"])`, `r sprintf("R^2^ = %.1f%%", 100*performance::r2(m_aspm_africa)$R2_marginal)`; *MCPH1*: `r sprintf("*p* = %.2g", anova(m_mcph1_0, m_mcph1_africa)[2,"Pr(>Chisq)"])`, `r sprintf("R^2^ = %.1f%%", 100*performance::r2(m_mcph1_africa)$R2_marginal)`), with lower frequencies in Africa.

Taken together, these suggest that the two "derived" allele frequencies are strongly confounded by *macroarea* (mostly Africa vs the rest of the world).
...
For these randomisation analyses there are several important parameters:
...
- *ASPM*-D:
  + not controlling for *macroarea* finds a negative effect on *tone1*, especially strong for unrestricted permutations, but also when restricting to within macroareas and families,
  + when having *macroarea* as a fixed effect, this negative effect on *tone1* is much reduced, but still present for unrestricted and within macroareas, but not within families.

- *MCPH1*-D, the negative effect is present only for the unrestricted permutations when not controlling for *macroarea*.
...
- a significant *positive* **total effect** of being in Africa on *tone1*, `r sprintf("%.2f (%.2f, %.2f), *p*=%.2g", med_aspm_summary$tau.coef, med_aspm_summary$tau.ci[1], med_aspm_summary$tau.ci[2], med_aspm_summary$tau.p)`, decomposed into:
...
- a significant *positive* **average indirect effect** (ACME) mediated by *ASPM*-D, `r sprintf("%.2f (%.2f, %.2f), *p*=%.2g", med_aspm_summary$d.avg, med_aspm_summary$d.avg.ci[1], med_aspm_summary$d.avg.ci[2], med_aspm_summary$d.avg.p)`, mediating `r sprintf("%.1f%% (%.1f%%, %.1f%%), *p*=%.2g", 100*med_aspm_summary$n.avg, 100*med_aspm_summary$n.avg.ci[1], 100*med_aspm_summary$n.avg.ci[2], med_aspm_summary$n.avg.p)` of the effect, resulting from:

  + a significant *negative* effect of being in Africa on *ASPM*-D, `r sprintf("%.2f ±%.2f, *p*=%.2g", med_aspm_f1_summary$coefficients["AfricaYes", "Estimate"], med_aspm_f1_summary$coefficients["AfricaYes", "Std. Error"], med_aspm_f1_summary$coefficients["AfricaYes", "Pr(>|t|)"])`, and
  + a significant *negative* effect of *ASPM*-D on *tone1*, `r sprintf("%.2f ±%.2f, *p*=%.2g", med_aspm_f2_summary$coefficients["ASPM_freq_wavg", "Estimate"], med_aspm_f2_summary$coefficients["ASPM_freq_wavg", "Std. Error"], med_aspm_f2_summary$coefficients["ASPM_freq_wavg", "Pr(>|z|)"])`.

For *MCPH1*-D, there are:

- a significant *positive* **total effect** of being in Africa on *tone1*, `r sprintf("%.2f (%.2f, %.2f), *p*=%.2g", med_mcph1_summary$tau.coef, med_mcph1_summary$tau.ci[1], med_mcph1_summary$tau.ci[2], med_mcph1_summary$tau.p)`, decomposed into:
...
- a *non-significant* **average indirect effect** (ACME) mediated by *MCPH1*, `r sprintf("%.2f (%.2f, %.2f), *p*=%.2g", med_mcph1_summary$d.avg, med_mcph1_summary$d.avg.ci[1], med_mcph1_summary$d.avg.ci[2], med_mcph1_summary$d.avg.p)`, mediating `r sprintf("%.1f%% (%.1f%%, %.1f%%), *p*=%.2g", 100*med_mcph1_summary$n.avg, 100*med_mcph1_summary$n.avg.ci[1], 100*med_mcph1_summary$n.avg.ci[2], med_mcph1_summary$n.avg.p)` of the effect, resulting from:
  ...
  + a significant *negative* effect of being in Africa on *MCPH1*-D, `r sprintf("%.2f ±%.2f, *p*=%.2g", med_mcph1_f1_summary$coefficients["AfricaYes", "Estimate"], med_mcph1_f1_summary$coefficients["AfricaYes", "Std. Error"], med_mcph1_f1_summary$coefficients["AfricaYes", "Pr(>|t|)"])`, and
  + a *non-significant* effect of *MCPH1*-D on *tone1*, `r sprintf("%.2f ±%.2f, *p*=%.2g", med_mcph1_f2_summary$coefficients["MCPH1_freq_wavg", "Estimate"], med_mcph1_f2_summary$coefficients["MCPH1_freq_wavg", "Std. Error"], med_mcph1_f2_summary$coefficients["MCPH1_freq_wavg", "Pr(>|z|)"])`.

Thus, this confirms that, while being in Africa has a major effect on *tone1*, this is partly mediated though *ASPM*-D but not through *MCHP1*-D.
...
- has a significant and strong positive direct effect on *tone1*,
- has a significant and strong negative effect on *ASPM*-D, which, in turn, has a negative effect on *tone1*,
- has a significant and very strong negative effect on *MCHP1*-D, which, however, has no effect on *tone1*.
...
Using the frequency of the two alleles and the *macroarea* as predictors, the decision tree fits the data pretty well: `r sprintf("accuracy = %.1f%%, sensitivity = %.1f%%, specificity = %.1f%%, precision = %.1f%%, and recall = %.1f%%", 100*success_ctree_tb_m_all["accuracy"], 100*success_ctree_tb_m_all["sensitivity"], 100*success_ctree_tb_m_all["specificity"], 100*success_ctree_tb_m_all["precision"], 100*success_ctree_tb_m_all["recall"])`.
...
On the `r n_train` training/testing sets, these models generalize pretty well, comparable to the success on the full data: `r success_ctree_tb_nom_traintest <- do.call(rbind, lapply(ctree_tb_nom_traintest, function(x) x$success)); paste0(names(success_ctree_tb_nom_traintest)[-1], " = ", vapply(success_ctree_tb_nom_traintest[,-1], function(x) sprintf("%.1f%% ±%.1f%%", mean(x,na.rm=TRUE)*100, sd(x,na.rm=TRUE)*100), character(1)), collapse=", ")`.
...
- with or without *macroarea*, the decision trees fit the full data well and generalize comparably well,
- with *macroarea*, the distinction between (Africa and Americas) vs (Eurasia and Papunesia) is the most important, followed, for the later, by the frequency of *ASPM*-D.
- without *macroarea*, the frequency of *ASPM*-D is the most important predictor of *tone1*, followed by the frequency of *MCHP1*-D when the frequency of *ASPM*-D is low (in fact, this seems to set aside Africa).
...
When using the frequency of the two alleles and the *macroarea* as predictors, the models fit the full data pretty well: 
...
When using the frequency of the two alleles only, the models fit the full data pretty well: 
...
- in decreasing order of their importance:
  + when including *macroarea*: we have *macroarea* as the most important once, and *ASPM*-D as the most important twice,
  + when excluding *macroarea*, *ASPM*-D is the most important all three times.
...
For *tone2* I used (mixed-effects) *logistic regression* (as implemented by `glmer()`), and for the allele frequencies I used (mixed-effects) *beta regression* (as implemented by `glmmTMB()`, after replacing all $0.0$ values by $10^{-7}$ and all $1.0$ by $1.0-10^{-7}$, respectively).
...
I fitted the "full model" of a mixed-effects logistic regression where *tone2* is predicted by *macroarea*, *ASPM*-D and *MCPH1*-D (with their *interactions*) as fixed effects, and language *family* as random effect, and I found that:

- the "null model" has `r sprintf("ICC = %.1f%%", 100*performance::icc(m_t3_0)$ICC_adjusted)`, which shows that *tone2* is very strongly clustered within families;
- the *interactions* drop out: there does not seem to be an interaction between the two alleles (`r sprintf("*p* = %.2g", anova(m_t3_full, m_t3_1)[2,"Pr(>Chisq)"])`), nor between the alleles and *macroarea* (`r sprintf("*p* = %.2g", anova(m_t3_full_old, m_t3_2)[2,"Pr(>Chisq)"])`);
...
- *ASPM*-D: there seems to be a negative effect in all conditions (except for permuting tone within families while controlling for macroarea),
- *MCPH1*-D: the negative effect is present only in the unrestricted case when not controlling for macroarea.
...
For either allele, there is no significant total effect: *ASPM*-D: `r sprintf("%.2f (%.2f, %.2f), *p*=%.2g", med_aspm_summary$tau.coef, med_aspm_summary$tau.ci[1], med_aspm_summary$tau.ci[2], med_aspm_summary$tau.p)`, *MCPH1*-D: `r sprintf("%.2f (%.2f, %.2f), *p*=%.2g", med_mcph1_summary$tau.coef, med_mcph1_summary$tau.ci[1], med_mcph1_summary$tau.ci[2], med_mcph1_summary$tau.p)`.
...
```{r fig.height=2*4, fig.width=3*4, fig.cap=capFig(paste0("Mediation analysis for ",n_samples," restricted samples (i.e., picking one random language per family). The leftmost panels show the distribution of point estimates of the Total Effect (TE), the Direct Effect (ADE) and the Indirect Effect (ACME) for *ASPM* and *MCHP1*; the middle panels show the distribution of the *p*-values for the same effects, while the rightmost panels show the distribution of the regression slopes (*β*) for the two alleles, top: for the regression of the allele frequency on within vs outside Africa, and bottom: for the regression of tone on the allele while controlling for within vs outside Africa. The black vertical lines show: 0.0 (solid), 0.05 (dashed) and 0.10 (dotted)."))}
p1 <- ggplot(reshape::melt(med_samples_per_fam[,c("aspm_TE", "aspm_ADE", "aspm_ACME")]),
                    aes(x=value, fill=variable), color="black") + xlim(-0.30, 0.75) +
               geom_density(alpha=0.3) + geom_vline(xintercept=0.0, color="black", linetype="solid") +
               scale_fill_discrete(name="Effect type", labels=c("aspm_TE"=expression(" "~italic("TE")), "aspm_ADE"=expression(" "~italic("ADE")), "aspm_ACME"=expression(" "~italic("ACME")))) + 
...
Thus, it seems that there is a *qualitative difference* between *ASPM*-D and *MCPH1*-D: while for both there are positive and comparably strong total (TE) and direct (ADE) effects, only *ASPM*-D shows a clearly positive indirect effect (ACME).
Moreover, being in Africa or not has a much stronger effect, *β*(Africa → allele), on *MCPH1*-D than on *ASPM*-D, and the partial effect of the allele on *tone2* after controlling for Africa, *β*(allele → tone | Africa), is clearly negative only for *ASPM*-D.
However, pretty much none of these effects are individually significant.
...
Coding Africa and *tone2* numerically, the model fits very well (`r sprintf("*&chi;*^2^(%d)=%.2f, *p*=%.2g; CFI=%.2f, TLI=%.2f, NNFI=%.2f and RFI=%.2f", semfit_tone_num_measures["df"], semfit_tone_num_measures["chisq"], semfit_tone_num_measures["pvalue"], semfit_tone_num_measures["cfi"], semfit_tone_num_measures["tli"], semfit_tone_num_measures["nnfi"], semfit_tone_num_measures["rfi"])`):
...
Likewise, codng Africa and *tone2* as ordered binary factors, the model fits very well (`r sprintf("*&chi;*^2^(%d)=%.2f, *p*=%.2g; CFI=%.2f, TLI=%.2f, NNFI=%.2f and RFI=%.2f", semfit_tone_ord_measures["df"], semfit_tone_ord_measures["chisq"], semfit_tone_ord_measures["pvalue"], semfit_tone_ord_measures["cfi"], semfit_tone_ord_measures["tli"], semfit_tone_ord_measures["nnfi"], semfit_tone_ord_measures["rfi"])`):
...
The two ways of coding the data result in very similar models, that suggest that being in Africa:

- has a non-significant direct effect on *tone2*, and this effect is mediated by
- the significant and strong negative effect on *ASPM*-D, which, in turn, has a negative effect on *tone2*.
...
- the path Africa → *tone2* is basically 0: `r sprintf("mean = %.2g, median = %.2g, sd = %.2g, IQR = %.2g, %.1f%% > 0; %.1f%% significant at *α*-level 0.05; one-sample one-sided *t*-test vs 0: *t*(%d) = %.2g, *p* = %.2g", mean(sem_samples_per_fam$Afr_tone_b, na.rm=TRUE), median(sem_samples_per_fam$Afr_tone_b, na.rm=TRUE), sd(sem_samples_per_fam$Afr_tone_b, na.rm=TRUE), IQR(sem_samples_per_fam$Afr_tone_b, na.rm=TRUE), 100*sum(sem_samples_per_fam$Afr_tone_b > 0, na.rm=TRUE)/nrow(sem_samples_per_fam), 100*sum(sem_samples_per_fam$Afr_tone_p < 0.05, na.rm=TRUE)/nrow(sem_samples_per_fam), (tt <- t.test(sem_samples_per_fam$Afr_tone_b, mu=0, alternative="greater"))$parameter, tt$statistic, tt$p.value)`;

- the path *ASPM*-D → *tone2* is negative: `r sprintf("mean = %.2g, median = %.2g, sd = %.2g, IQR = %.2g, %.1f%% < 0; %.1f%% significant at *α*-level 0.05; one-sample one-sided *t*-test vs 0: *t*(%d) = %.2g, *p* = %.2g", mean(sem_samples_per_fam$aspm_tone_b, na.rm=TRUE), median(sem_samples_per_fam$aspm_tone_b, na.rm=TRUE), sd(sem_samples_per_fam$aspm_tone_b, na.rm=TRUE), IQR(sem_samples_per_fam$aspm_tone_b, na.rm=TRUE), 100*sum(sem_samples_per_fam$aspm_tone_b < 0, na.rm=TRUE)/nrow(sem_samples_per_fam), 100*sum(sem_samples_per_fam$aspm_tone_p < 0.05, na.rm=TRUE)/nrow(sem_samples_per_fam), (tt <- t.test(sem_samples_per_fam$aspm_tone_b, mu=0, alternative="less"))$parameter, tt$statistic, tt$p.value)`;

- the path *MCPH1*-D → *tone2* tends slightly to be negative: `r sprintf("mean = %.2g, median = %.2g, as = %.2g, IQR = %.2g, %.1f%% < 0; %.1f%% significant at *α*-level 0.05; one-sample one-sided *t*-test vs 0: *t*(%d) = %.2g, *p* = %.2g", mean(sem_samples_per_fam$mcph1_tone_b, na.rm=TRUE), median(sem_samples_per_fam$mcph1_tone_b, na.rm=TRUE), sd(sem_samples_per_fam$mcph1_tone_b, na.rm=TRUE), IQR(sem_samples_per_fam$mcph1_tone_b, na.rm=TRUE), 100*sum(sem_samples_per_fam$mcph1_tone_b < 0, na.rm=TRUE)/nrow(sem_samples_per_fam), 100*sum(sem_samples_per_fam$mcph1_tone_p < 0.05, na.rm=TRUE)/nrow(sem_samples_per_fam), (tt <- t.test(sem_samples_per_fam$mcph1_tone_b, mu=0, alternative="less"))$parameter, tt$statistic, tt$p.value)`.

Thus, again, there is a negative effect of *ASPM*-D on *tone2*, there is no effect of being in Africa or not, but possibly a negative one for *MCPH1*-D.
...
When using the frequency of the two alleles and the *macroarea* as predictors, the decision tree is trivial (in that it uniformly predicts just the majority value "No"):
...
So, this is not very informative (except in that it takes the frequency of *ASPM*-D as the only predictor that would matter).
...
So, the random forests fit the data comparably well with or without macroarea, and *ASPM*-D is always the most important predictor, followed by *MCHP1*-D and *macroarea*.
...
I used a mixed-effects Poisson model (as implemented by `glmer()`) where tone *counts* are predicted by *macroarea* and the two alleles (linear and quadratic but no interactions) when considering language *family* as random effect, and I found that:

- for the "null model", the Poisson model is not overdispersed (`r sprintf("*&chi;*^2^(%d) = %.1f, *p* = %.2g", overdisp_fun(m_tc_0)["rdf"], overdisp_fun(m_tc_0)["chisq"], overdisp_fun(m_tc_0)["p"])`), and `r sprintf("ICC = %.1f%%", 100*performance::icc(m_tc_0)$ICC_adjusted)`, which shows that tone *counts* are clustered within families;
...
- *ASPM*-D has a non-significant negative effect on tone *counts* (`r sprintf("*β* = %.2g ± %.2g, *p* = %.2g", summary(m_tc_aspm_1)$coefficients["ASPM_freq_wavg", "Estimate"], summary(m_tc_aspm_1)$coefficients["ASPM_freq_wavg", "Std. Error"], anova(m_tc_0, m_tc_aspm_1)[2,"Pr(>Chisq)"])`, `r sprintf("R^2^ = %.1f%%", 100*performance::r2(m_tc_aspm_1)$R2_marginal)`);
- *MCPH1*-D has a significant negative linear effect on tone *counts* (`r sprintf("*β* = %.2g ± %.2g, *p* = %.2g", summary(m_tc_mcph1_1)$coefficients["MCPH1_freq_wavg", "Estimate"], summary(m_tc_mcph1_1)$coefficients["MCPH1_freq_wavg", "Std. Error"], anova(m_tc_0, m_tc_mcph1_1)[2,"Pr(>Chisq)"])`, `r sprintf("R^2^ = %.1f%%", 100*performance::r2(m_tc_mcph1_1)$R2_marginal)`), but this seems to fully overlap that of *macroarea*.
...
So, for both alleles, there seems to be a negative effect when not controlling for macroarea and family.
...
- a significant *positive* **total effect** of being in Africa on tone *counts*, `r sprintf("%.2f (%.2f, %.2f), *p*=%.2g", med_aspm_summary$tau.coef, med_aspm_summary$tau.ci[1], med_aspm_summary$tau.ci[2], med_aspm_summary$tau.p)`, decomposed into:
...
- a significant *positive* **average indirect effect** (ACME) mediated by *ASPM*-D, `r sprintf("%.2f (%.2f, %.2f), *p*=%.2g", med_aspm_summary$d.avg, med_aspm_summary$d.avg.ci[1], med_aspm_summary$d.avg.ci[2], med_aspm_summary$d.avg.p)`, mediating `r sprintf("%.1f%% (%.1f%%, %.1f%%), *p*=%.2g", 100*med_aspm_summary$n.avg, 100*med_aspm_summary$n.avg.ci[1], 100*med_aspm_summary$n.avg.ci[2], med_aspm_summary$n.avg.p)` of the effect, resulting from:
  ...
  + a significant *negative* effect of being in Africa on *ASPM*-D, `r sprintf("%.2f ±%.2f, *p*=%.2g", med_aspm_f1_summary$coefficients["AfricaYes", "Estimate"], med_aspm_f1_summary$coefficients["AfricaYes", "Std. Error"], med_aspm_f1_summary$coefficients["AfricaYes", "Pr(>|t|)"])`, and
  + a significant *negative* effect of *ASPM*-D on tone *counts*, `r sprintf("%.2f ±%.2f, *p*=%.2g", med_aspm_f2_summary$coefficients["ASPM_freq_wavg", "Estimate"], med_aspm_f2_summary$coefficients["ASPM_freq_wavg", "Std. Error"], med_aspm_f2_summary$coefficients["ASPM_freq_wavg", "Pr(>|z|)"])`.

For *MCPH1*-D, there are:

- a significant *positive* **total effect** of being in Africa on tone *counts*, `r sprintf("%.2f (%.2f, %.2f), *p*=%.2g", med_mcph1_summary$tau.coef, med_mcph1_summary$tau.ci[1], med_mcph1_summary$tau.ci[2], med_mcph1_summary$tau.p)`, decomposed into:
...
- a *non-significant* **average indirect effect** (ACME) mediated by *MCPH1*-D, `r sprintf("%.2f (%.2f, %.2f), *p*=%.2g", med_mcph1_summary$d.avg, med_mcph1_summary$d.avg.ci[1], med_mcph1_summary$d.avg.ci[2], med_mcph1_summary$d.avg.p)`, mediating `r sprintf("%.1f%% (%.1f%%, %.1f%%), *p*=%.2g", 100*med_mcph1_summary$n.avg, 100*med_mcph1_summary$n.avg.ci[1], 100*med_mcph1_summary$n.avg.ci[2], med_mcph1_summary$n.avg.p)` of the effect.

Thus, this mediation analysis confirms that, while being in Africa has an effect on tone *counts*, this is partly mediated though *ASPM*-D but not through *MCHP1*-D.
...
Thus, it is a *qualitative difference* between *ASPM*-D and *MCPH1*-D: while for both there are positive and comparably strong total (TE) and direct (ADE) effects, only *ASPM*-D shows a clearly positive indirect effect (ACME).
Moreover, being in Africa or not has a much stronger effect, *β*(Africa → allele), on *MCPH1*-D than on *ASPM*-D, and the partial effect of the allele on tone *counts*, after controlling for Africa, *β*(allele → tone | Africa), is clearly negative only for *ASPM*-D.
...
- the significant and strong negative effect on *ASPM*-D, which, in turn, has a negative effect on tone *counts*.
...
- the path Africa → tone *counts* is positive: `r sprintf("mean = %.2g, median = %.2g, sd = %.2g, IQR = %.2g, %.1f%% > 0; %.1f%% significant at *α*-level 0.05; one-sample one-sided *t*-test vs 0: *t*(%d) = %.2g, *p* = %.2g", mean(sem_samples_per_fam$Afr_tone_b, na.rm=TRUE), median(sem_samples_per_fam$Afr_tone_b, na.rm=TRUE), sd(sem_samples_per_fam$Afr_tone_b, na.rm=TRUE), IQR(sem_samples_per_fam$Afr_tone_b, na.rm=TRUE), 100*sum(sem_samples_per_fam$Afr_tone_b > 0, na.rm=TRUE)/nrow(sem_samples_per_fam), 100*sum(sem_samples_per_fam$Afr_tone_p < 0.05, na.rm=TRUE)/nrow(sem_samples_per_fam), (tt <- t.test(sem_samples_per_fam$Afr_tone_b, mu=0, alternative="greater"))$parameter, tt$statistic, tt$p.value)`;

- the path *ASPM*-D → tone *counts* is negative: `r sprintf("mean = %.2g, median = %.2g, sd = %.2g, IQR = %.2g, %.1f%% < 0; %.1f%% significant at *α*-level 0.05; one-sample one-sided *t*-test vs 0: *t*(%d) = %.2g, *p* = %.2g", mean(sem_samples_per_fam$aspm_tone_b, na.rm=TRUE), median(sem_samples_per_fam$aspm_tone_b, na.rm=TRUE), sd(sem_samples_per_fam$aspm_tone_b, na.rm=TRUE), IQR(sem_samples_per_fam$aspm_tone_b, na.rm=TRUE), 100*sum(sem_samples_per_fam$aspm_tone_b < 0, na.rm=TRUE)/nrow(sem_samples_per_fam), 100*sum(sem_samples_per_fam$aspm_tone_p < 0.05, na.rm=TRUE)/nrow(sem_samples_per_fam), (tt <- t.test(sem_samples_per_fam$aspm_tone_b, mu=0, alternative="less"))$parameter, tt$statistic, tt$p.value)`;

- the path *MCPH1*-D → tone *counts* is basically 0: `r sprintf("mean = %.2g, median = %.2g, as = %.2g, IQR = %.2g, %.1f%% < 0; %.1f%% significant at *α*-level 0.05; one-sample one-sided *t*-test vs 0: *t*(%d) = %.2g, *p* = %.2g", mean(sem_samples_per_fam$mcph1_tone_b, na.rm=TRUE), median(sem_samples_per_fam$mcph1_tone_b, na.rm=TRUE), sd(sem_samples_per_fam$mcph1_tone_b, na.rm=TRUE), IQR(sem_samples_per_fam$mcph1_tone_b, na.rm=TRUE), 100*sum(sem_samples_per_fam$mcph1_tone_b < 0, na.rm=TRUE)/nrow(sem_samples_per_fam), 100*sum(sem_samples_per_fam$mcph1_tone_p < 0.05, na.rm=TRUE)/nrow(sem_samples_per_fam), (tt <- t.test(sem_samples_per_fam$mcph1_tone_b, mu=0, alternative="less"))$parameter, tt$statistic, tt$p.value)`.

Thus, again, there is a negative effect of *ASPM*-D on tone *counts*, there is a positive effect of being in Africa, but no effect for *MCPH1*-D.

...
- include the genetic and linguistic processing
- remove Sheelece's data from here
- keep only tone (and not syllable structure)
...
# Relative path to the sister project on tone & syllable structure were the data pre-processing was done:
.path_to_tone_syllstr <- "../../Syllable complexity + tone/";
...
load(paste0(.path_to_tone_syllstr,"code/cache-results/language_data_scaled.RData"));

# Add human-readable family name:
glottolog_data <- read.table("../../Syllable complexity + tone/data/language/input/originals/glottolog4.1/languoid.csv", header=TRUE, sep=",", quote='"', stringsAsFactors=FALSE);
language_data_scaled <- merge(language_data_scaled, glottolog_data[,c("id", "name")], by.x="family", by.y="id", all.x=TRUE, all.y=FALSE, suffixes=c("","_family"));
names(language_data_scaled)[ncol(language_data_scaled)] <- "family_name";
...
data_all <- merge(language_data_scaled, genetic_data, by.x=c("iso"), by.y=c("ISO_639_3"), suffixes=c("_L", "_G"), all=FALSE);
data_all <- unique(data_all);

# z-score the (weigthed average) allele frequencies:
...
data_all <- data_all[ order(data_all$macroarea, data_all$Metapopulation_name, data_all$name, data_all$iso, data_all$Pop_UID) ,
                      c("Metapopulation_name", "Metapopulation_ID", "Pop_UID", # metapopulation population, and sample info
                         ...
                         "macroarea", "latitude", "longitude", "longitude_shifted", "country", # geographical info
                         "tone_binary", "tone_3way", "n_tones", # agreement tone data
                         "aspm", "mcph1", # ASPM and MCPH1 z-scored frequency (weigthed averages)
                         "ph_n_tones_original", "wa_tone", "la_tone", "la_n_tones_original", "la_tone_comments", "se_tone", "dl_tone", "wp_tone_original", # original tone data
...
We use here the agreement between multiple individual databases (WALS, LAPSyD, Shelece Easterday's, Dediu & Ladd (2007), PHOIBLE, and World Phonotactis Database) encoded in three variables:
...
m_tb_1 <- update(m_tb_full, . ~ . - ASPM_freq_wavg:MCPH1_freq_wavg); # Model failed to converge: degenerate  Hessian with 1 negative eigenvalues
summary(m_tb_1); anova(m_tb_full, m_tb_1); # p=0.8412
m_tb_2 <- update(m_tb_1, . ~ . - ASPM_freq_wavg:macroarea - MCPH1_freq_wavg:macroarea);
summary(m_tb_2); anova(m_tb_full, m_tb_2); # p=0.2457
...
summary(m_tb_3); anova(m_tb_2, m_tb_3); # p=0.2827 -> no interactions!
m_tb_full <- m_tb_2;
...
anova(m_tb_full, m_tb_nom); # p=0.04378 * -> yes
anova(m_tb_0, m_tb_nom); # p=0.01098 * -> but the genes add info!

# Do the genes matter?
m_tb_nog <- update(m_tb_full, . ~ . - ASPM_freq_wavg - MCPH1_freq_wavg);
summary(m_tb_nog);
performance::icc(m_tb_nog); performance::r2(m_tb_nog);
anova(m_tb_full, m_tb_nog); # p=0.9107 -> no
anova(m_tb_0, m_tb_nog); # p=0.0007249 *** -> and macroarea adds info!
# ASPM:
m_tb_aspm_m <- update(m_tb_nog, . ~ . + ASPM_freq_wavg);
summary(m_tb_aspm_m); anova(m_tb_nog, m_tb_aspm_m); # p=0.6731
m_tb_aspm <- update(m_tb_aspm_m, . ~ . - macroarea);
summary(m_tb_aspm); 
performance::icc(m_tb_aspm); performance::r2(m_tb_aspm);
anova(m_tb_aspm_m, m_tb_aspm); # p=0.02011 * -> macroarea adds to ASPM
anova(m_tb_0, m_tb_aspm); # p=0.006899 ** -> ASPM predicts tone by itself
# MCPH1:
m_tb_mcph1_m <- update(m_tb_nog, . ~ . + MCPH1_freq_wavg);
summary(m_tb_mcph1_m); anova(m_tb_nog, m_tb_mcph1_m); # p=0.923
m_tb_mcph1 <- update(m_tb_mcph1_m, . ~ . - macroarea);
summary(m_tb_mcph1); 
performance::icc(m_tb_mcph1); performance::r2(m_tb_mcph1);
anova(m_tb_mcph1_m, m_tb_mcph1); # p=0.01228 * -> macroarea adds to MCPH1
anova(m_tb_0, m_tb_mcph1); # p=0.01385 * -> MCPH1 predicts tone by itself
...
- the interactions drop out (i.e., there does not seem to be an interaction between the two genes, not between the genes and macroarea);
...
- the two genes together predict tone (`r sprintf("*p* = %.2g", anova(m_tb_0, m_tb_nom)[2,"Pr(>Chisq)"])`, `r sprintf("R^2^ = %.1f%%", 100*performance::r2(m_tb_nom)$R2_marginal)`): 
  + *ASPM* by itself has a negative significant effect (`r sprintf("*β* = %.2g ± %.2g, *p* = %.2g", summary(m_tb_aspm)$coefficients["ASPM_freq_wavg", "Estimate"], summary(m_tb_aspm)$coefficients["ASPM_freq_wavg", "Std. Error"], anova(m_tb_0, m_tb_aspm)[2,"Pr(>Chisq)"])`, `r sprintf("R^2^ = %.1f%%", 100*performance::r2(m_tb_aspm)$R2_marginal)`), 
  + as does *MCPH1* by itself (`r sprintf("*β* = %.2g ± %.2g, *p* = %.2g", summary(m_tb_mcph1)$coefficients["MCPH1_freq_wavg", "Estimate"], summary(m_tb_mcph1)$coefficients["MCPH1_freq_wavg", "Std. Error"], anova(m_tb_0, m_tb_mcph1)[2,"Pr(>Chisq)"])`, `r sprintf("R^2^ = %.1f%%", 100*performance::r2(m_tb_mcph1)$R2_marginal)`);
- however, macroarea and the two genes seem to carry pretty much the same information about tone (i.e., their effects when controlling for the others are not different from 0)...
...
anova(m_mcph1_0, m_mcph1_macroarea); # p=3.14e-12 ***

# Just Africa vs the rest:
try(m_mcph1_africa <- update(m_mcph1_0, . ~ . + Africa), silent=FALSE);
summary(m_mcph1_africa);
anova(m_mcph1_0, m_mcph1_africa); # p=3.288e-09 ***
...
However, conducting separate analyses in Africa and Eurasia finds no fixed effect for the two genes (except for Eurasia when we do not control for family).
...
# check predictors:
m_tb_africa_rest_1 <- update(m_tb_africa_rest, . ~ . - ASPM_freq_wavg:MCPH1_freq_wavg); summary(m_tb_africa_rest_1); anova(m_tb_africa_rest, m_tb_africa_rest_1); # p=0.4464
m_tb_africa_rest_2 <- update(m_tb_africa_rest_1, . ~ . - MCPH1_freq_wavg:Africa); summary(m_tb_africa_rest_2); anova(m_tb_africa_rest_1, m_tb_africa_rest_2); # p=0.5888
m_tb_africa_rest_3 <- update(m_tb_africa_rest_2, . ~ . - MCPH1_freq_wavg); summary(m_tb_africa_rest_3); anova(m_tb_africa_rest_2, m_tb_africa_rest_3); # p=0.2097
anova(m_tb_africa_rest, m_tb_africa_rest_3); # p=0.4852 -> no effect of MCPH1
# check ASPM specifically:
m_tb_africa_rest_4 <- update(m_tb_africa_rest_3, . ~ . - ASPM_freq_wavg - ASPM_freq_wavg:Africa); summary(m_tb_africa_rest_4); anova(m_tb_africa_rest_3, m_tb_africa_rest_4); # p=0.3474
# check Africa specifically:
m_tb_africa_rest_5 <- update(m_tb_africa_rest_3, . ~ . - Africa - ASPM_freq_wavg:Africa); summary(m_tb_africa_rest_5); anova(m_tb_africa_rest_3, m_tb_africa_rest_5); # p=0.01123 *
...
anova(m_tb_africa_full, m_tb_africa_noint, test="Chisq"); # p=0.08549 . -> no gene x gene interaction
anova(m_tb_africa_0, m_tb_africa_noint, test="Chisq"); # p=0.557 -> no effect of the genes
# ASPM:
m_tb_africa_aspm <- update(m_tb_africa_0, . ~ . + ASPM_freq_wavg);
summary(m_tb_africa_aspm); anova(m_tb_africa_0, m_tb_africa_aspm, test="Chisq"); # p=0.2933 -> nope
# MCPH1:
m_tb_africa_mcph1 <- update(m_tb_africa_0, . ~ . + MCPH1_freq_wavg);
summary(m_tb_africa_mcph1); anova(m_tb_africa_0, m_tb_africa_mcph1, test="Chisq"); # p=0.3444 -> nope
# -> no effect within Africa
...
anova(m_tb_eurasia_full, m_tb_eurasia_noint); # p=0.6239 -> no gene x gene interaction
anova(m_tb_eurasia_0, m_tb_eurasia_noint); # p=0.6151 -> no effect of the genes
# ASPM:
m_tb_eurasia_aspm <- update(m_tb_eurasia_0, . ~ . + ASPM_freq_wavg);
summary(m_tb_eurasia_aspm); anova(m_tb_eurasia_0, m_tb_eurasia_aspm); # p=0.3312 -> nope
# MCPH1:
m_tb_eurasia_mcph1 <- update(m_tb_eurasia_0, . ~ . + MCPH1_freq_wavg);
summary(m_tb_eurasia_mcph1); anova(m_tb_eurasia_0, m_tb_eurasia_mcph1); # p=0.7393 -> nope
# -> no effect within Eurasia

# Within Eurasia (no random effects):
m_tb_eurasia_full <- glm(tone_binary ~ 1 + # intercept
                            ASPM_freq_wavg * MCPH1_freq_wavg, # the two genes (and their interaction)
...
m_tb_eurasia_noint <- update(m_tb_eurasia_full, . ~ . - ASPM_freq_wavg:MCPH1_freq_wavg);
m_tb_eurasia_0     <- update(m_tb_eurasia_noint, . ~ . - ASPM_freq_wavg - MCPH1_freq_wavg);
summary(m_tb_eurasia_noint); 
anova(m_tb_eurasia_full, m_tb_eurasia_noint, test="Chisq"); # p=0.8401 -> no gene x gene interaction
anova(m_tb_eurasia_0, m_tb_eurasia_noint, test="Chisq"); # p=0.0005596 *** -> clear effect of the genes
# ASPM:
m_tb_eurasia_aspm <- update(m_tb_eurasia_0, . ~ . + ASPM_freq_wavg);
summary(m_tb_eurasia_aspm); anova(m_tb_eurasia_0, m_tb_eurasia_aspm, test="Chisq"); # 0.0001173 *** -> yes
# MCPH1:
m_tb_eurasia_mcph1 <- update(m_tb_eurasia_0, . ~ . + MCPH1_freq_wavg);
summary(m_tb_eurasia_mcph1); anova(m_tb_eurasia_0, m_tb_eurasia_mcph1, test="Chisq"); # p=0.7096 -> nope
...
- *ASPM*:
  + not controlling for macroareas finds a negative effect on tone, especially strong for unrestricted permutations, but when restricted to macroareas and families, but
...
n_samples <- 1000;
reg_samples_per_fam <- do.call(rbind, lapply(1:n_samples, function(i)
{
  # Sample:
...
```{r fig.cap=capFig(paste0("Results of one sample per family for *ASPM*: ", round(100*sum(reg_samples_per_fam$b_aspm<0)/nrow(reg_samples_per_fam),1), "% of &beta;s are negative when regressing tone on *ASPM* alone, ", round(100*sum(reg_samples_per_fam$b_aspm_macroarea<0)/nrow(reg_samples_per_fam),1), "% when controlling for the macroarea, and ", round(100*sum(reg_samples_per_fam$b_aspm_all<0)/nrow(reg_samples_per_fam),1), "% when controlling for both macroarea and *MCHP1*."))}
...
```{r fig.cap=capFig(paste0("Results of one sample per family for *MCHP1*: ", round(100*sum(reg_samples_per_fam$b_mcph1<0)/nrow(reg_samples_per_fam),1), "% of &beta;s are negative when regressing tone on *MCHP1* alone, ", round(100*sum(reg_samples_per_fam$b_mcph1_macroarea<0)/nrow(reg_samples_per_fam),1), "% when controlling for the macroarea, and ", round(100*sum(reg_samples_per_fam$b_mcph1_all<0)/nrow(reg_samples_per_fam),1), "% when controlling for both macroarea and *ASPM*."))}
...
- has a significant and extremely strong negative effect on *MCHP1*, which, however, has no effect on tone.
...
b) the two genes have no effect within Africa (not surprising given the distribution of tone and the two genes), nor within Eurasia (except when we do not control for language family, which is expected given the discussion above about time depth and small effective sample size),
...
d) restricted sampling (extracting only one language per family) shows again that *ASPM* has a negative effect on tone even after controlling for macroarea (but not for *MCPH1*),
...
Thus, it seems that ASPM has a weak negative influence on tone above and beyond the effects of macroarea and language family, but these effects are very tricky to disentangle; on the other hand, it is seems that the effect of MCPH1 is entirely artifactual and due to its very skewed distribution within and outside Africa.



## Tone 3-way (complex vs none+simple; "tone2")
...
m_t3_full <- glmer(tone_complex ~ 1 + # intercept
                     ...
                     (1 | family_name), # random effects structure
                   family=binomial(), data=d_t3, control=glmer_ctrl);
summary(m_t3_full); # Hessian is numerically singular: parameters are not uniquely determined -> try to remove interactions:
m_t3_1 <- update(m_t3_full, . ~ . - ASPM_freq_wavg:MCPH1_freq_wavg); # Model failed to converge: degenerate  Hessian with 2 negative eigenvalues
summary(m_t3_1); anova(m_t3_full, m_t3_1); # p=0.6044
m_t3_2 <- update(m_t3_1, . ~ . - ASPM_freq_wavg:macroarea - MCPH1_freq_wavg:macroarea);
summary(m_t3_2); anova(m_t3_full, m_t3_2); # p=0.5045
# -> ok, so this seems to be the base model... try again the gene-gene interaction:
m_t3_3 <- update(m_t3_2, . ~ . + ASPM_freq_wavg:MCPH1_freq_wavg); # Model is nearly unidentifiable: large eigenvalue ratio
summary(m_t3_3); anova(m_t3_2, m_t3_3); # p=0.1198 -> no interactions!
m_t3_full <- m_t3_2;
...
summary(m_t3_nom);
anova(m_t3_full, m_t3_nom); # p=0.9322 -> no
anova(m_t3_0, m_t3_nom); # p=0.054 . -> the genes don't really add much...

# Do the genes matter?
m_t3_nog <- update(m_t3_full, . ~ . - ASPM_freq_wavg - MCPH1_freq_wavg);
summary(m_t3_nog); anova(m_t3_full, m_t3_nog); # p=0.5291 -> no
anova(m_t3_0, m_t3_nog); # p=0.1716 -> macroarea adds no info
# ASPM:
m_t3_aspm_m <- update(m_t3_nog, . ~ . + ASPM_freq_wavg);
summary(m_t3_aspm_m); anova(m_t3_nog, m_t3_aspm_m); # p=0.2937
m_t3_aspm <- update(m_t3_aspm_m, . ~ . - macroarea);
summary(m_t3_aspm); 
anova(m_t3_aspm_m, m_t3_aspm); # p=0.7388 -> macroarea does not add info
anova(m_t3_0, m_t3_aspm); # p=0.0277 * -> ASPM predicts tone by itself
# MCPH1:
m_t3_mcph1_m <- update(m_t3_nog, . ~ . + MCPH1_freq_wavg);
summary(m_t3_mcph1_m); anova(m_t3_nog, m_t3_mcph1_m); # p=0.3702
m_t3_mcph1 <- update(m_t3_mcph1_m, . ~ . - macroarea);
summary(m_t3_mcph1); 
anova(m_t3_mcph1_m, m_t3_mcph1); # p=0.7796 -> macroarea doesn not add info
anova(m_t3_0, m_t3_mcph1); # p=0.02987 * -> MCPH1 predicts tone by itself

# -> the affect of ASPM and MCPH1 seems fully accounted for by macroarea, but are predictive for complex tone!
```
...
- the two genes together almost predict tone (`r sprintf("*p* = %.2g", anova(m_t3_0, m_t3_nom)[2,"Pr(>Chisq)"])`, `r sprintf("R^2^ = %.1f%%", 100*performance::r2(m_t3_nom)$R2_marginal)`): 
  + *ASPM* by itself has a negative significant affect (`r sprintf("*β* = %.2g ± %.2g, *p* = %.2g", summary(m_t3_aspm)$coefficients["ASPM_freq_wavg", "Estimate"], summary(m_t3_aspm)$coefficients["ASPM_freq_wavg", "Std. Error"], anova(m_t3_0, m_t3_aspm)[2,"Pr(>Chisq)"])`, `r sprintf("R^2^ = %.1f%%", 100*performance::r2(m_t3_aspm)$R2_marginal)`), 
  + as does *MCPH1* by itself (`r sprintf("*β* = %.2g ± %.2g, *p* = %.2g", summary(m_t3_mcph1)$coefficients["MCPH1_freq_wavg", "Estimate"], summary(m_t3_mcph1)$coefficients["MCPH1_freq_wavg", "Std. Error"], anova(m_t3_0, m_t3_mcph1)[2,"Pr(>Chisq)"])`, `r sprintf("R^2^ = %.1f%%", 100*performance::r2(m_t3_mcph1)$R2_marginal)`);
- however, macroarea and the two genes seem to carry pretty much the same information about tone (i.e., their effects when controlling for the others are not different from 0)...
...
- *ASPM*, there seems to be a negative effect in all conditions,
- *MCPH1*, the negative effect is present only when not controlling for macroarea.
...
For *MCPH1*, there a *non-significant* **total effect** of being in Africa on tone, `r sprintf("%.2f (%.2f, %.2f), *p*=%.2g", med_mcph1_summary$tau.coef, med_mcph1_summary$tau.ci[1], med_mcph1_summary$tau.ci[2], med_mcph1_summary$tau.p)`.

Thus, this mediation analysis conforms that, while being in Africa has a major effect on complex tone, this is partly mediated though *ASPM* but not through *MCHP1*.
...
Complex tone is much more rare than no tone + simple tone, but:

- *ASPM* and *MCPH1* have, by themselves, negative effects on complex tone, but these effects are confounded, to a much lesser degree, by macroarea;
- the negative effect of *ASPM* (but not of *MCHP1*) is present when permuting the data and when doing restricted sampling (one language per family) even after controlling for macroarea;
- both mediation and math analysis show that the effect of macroarea is fully mediated by the negative effect by *ASPM* (but not by *MCPH1*);
- the decision trees and random forests find that *ASPM* is the most important predictor of complex tone.

Thus, when it comes to complex tone, *ASPM* has a negative effect that goes beyond the influences of macroarea and language family.
...
- however, the randomization and especially the restricted sampling clearly find a negative effect of *ASPM* on tone counts;
- both mediation and math analysis find that the effect of macroarea is fully mediated by the negative effect by *ASPM* (but not by *MCPH1*).
...
The main limiting factor, just as in 2006-2007 [@dediu_ladd_2007;@dediu_phd_2007], is the availability of *frequency data* for the "derived" alleles of *ASPM* and *MCPH1* (*Microcephalin*).
Back in 2007 we had to use a subset of 49 populations from the 59 in the original @mekel_bobrov_ongoing_2005 and @evans_microcephalin_2005 (see @dediu_ladd_2007 for details) with a very uneven geographic spread, and linguistic, cultural and demographic/"ethnic" representativeness.
For this study, we were able to use several new databases and publications (as detailed in the [Data section](#aspm-and-mcph1-population-frequencyes)) but we encountered these problems:

1) some sources do not contain frequency data about the "derived" alleles themselves (or the corresponding SNPs), so we had to use instead other loci (SPNs) in strong linkage disequilibrium (LD); however, LD might vary between populations, which means that there could be undetected decorrelations between these proxy SPNs and the target alleles in some populations (especially in those that are under-represented in the [LDLink](https://ldlink.nci.nih.gov/?tab=home) database which was used to select these proxy SNPs), resulting in a weakening of the signal;
2) some of these "genetic samples" are quite loosely defined, and reflect the mostly ad-hoc, opportunistic or "by-product" nature of the genetic sampling, generating the following related issues:
  1) some of these "samples" may come from the same "(meta)population", but this is not always easy to ascertain (e.g., for "Mbuti" there are two samples, for "Adygei" three, and for "Han" five) and might not mean the same thing for different (meta)populations (e.g, "Adygei" vs "African American" vs "Han"), and
  2) for certain samples and (meta)populations, it is far clear which ethno-linguistic group(s) precisely were sampled, resulting sometimes in multiple languages being assigned (e.g., for "San" there is a single sample bu the description does not allow a choice between several languages: East Taa [huaa1248], Hai//om-Akhoe [haio1238], Nama (Namibia) [nama1264], North-Central Ju [kung1261] and South-Eastern Ju [juho1239]), languages which may or may not differ in the linguistic feature of interest (here, tone).
...
The second limiting factor is the availability and reliability of data concerning *linguistic tone*: while in 2006-2007 [@dediu_ladd_2007;@dediu_phd_2007] we performed the binary judgments (i.e., "is there a tone system in the language?", "yes" or "no") ourselves based on several secondary (such as @wals_2005 and @compendium_2000) and primary sources (including by sending questionnaires to specialists in various languages), here we reconciled data from multiple databases (see [Data section](#tone)).
However, this reconciliation is sometimes not trivial, and involved a subjective hierarchy of "trust" that would prioritise these databases in the case of conflicts.
Moreover, we have collected here not only the binary "no tone" vs "tone" judgments, but also a three-way ordered distinction between "no tone" < "simple tone" < "complex tone", as well as the actual count of the tones (or "tone symbols") if available.
Thus, we think that the tone data used here should be relatively reliable.

So, in fact, we have `r nrow(data_all)` datapoints in total (some with various degrees of missing data)), of which there are `r length(unique(data_all$pop_ID))` unique genetic samples (with data for 7 loci for *ASPM* and 3 for *MCHP1*) distributed in `r length(unique(data_all$metapop))` (meta)populations, matched to `r length(unique(data_all$glottocode))` unique Glottolog codes.
...
Overall, the methods we used here do suggest that the frequency of the "derived" allele of *ASPM* in a population has a negative effect on use of tone in the language(s) spoken by that population, while the evidence for an effect of *MCHP1* is much weaker (in fact, absent).
This negative effect of *ASPM* is present for both binarisations of tone (i.e., "tone" versus "no tone", and "complex tone" versus "simple or no tone") and for tone counts (but much less clear), but it is shadowed by the effects of the macroarea and of language families.

More precisely, *macroarea* (and especially the contrast between Africa and non-Africa) explains ~20% of the distribution of tone1 and tone counts (but not of tone2), and ~60% of the distribution of *ASPM* and ~70% of that of *MCPH1*.
There is no surprise that tone has a continent-scale pattern of distribution (but it is surprising that complex tone seems not to, in our data), and that the two genes have widely unequal distributions inside and outside Africa (this is how they were selected for the original papers [@mekel_bobrov_ongoing_2005;@evans_microcephalin_2005] and why they attracted our attention in the first place [@dediu_ladd_2007;@ladd_biolinguistics_2008]), but what does this mean?
Likewise, ~70% of tone1, ~95% of tone2, ~65% of tone counts, and 70% and 100% of the "derived" allele frequencies of *ASPM* and *MCHP1*, respectively, is due to clustering within *language families* (i.e., related languages have a very strong tendency to be similar to each other presumably due to their shared ancestry), which is not surprising give the stability of tone [@dediu_bayesian_2011;@dediu_structural_2013], on the one hand, and the mode of evolution of these two alleles (see below).

A "blind" statistical approach (rather standard now) it is to "partial out" for the effects of macroarea and language family on the relationship between the two genes and tone, and to only focus on the reaming partial effects of the genes on tone.
One popular way of implementing this is to have macroarea as a fixed effect, and family as a random effect in a regression models, as we did here, in which case there is almost no extra explanatory role of genes besides that of macroarea.
While this can be interpreted as "there's nothing causally interesting about these genes" because macroarea and family explains everything, it is probably not the correct interpretation here.
The problem is that macroareas (leaving aside their rather arbitrary definitions variously used in the literature -- we used here the ones in Glottolog simply because they tend to be seen as "standard") and language families by themselves do not explain anything, but are rather proxies for demographic and linguistic processes and events that concern deep timescales and large spatial areas.
It is these processes and events that shape (in part) the observed linguistic and genetic diversities and that can create spurious correlations between them, as when, for example, a demographic expansion (say, due to agriculture) creates correlations between the languages and genes of the people that expand simply because humans tend to transmit both to their children.

However, this can easily hide a *causal relationship* of the type proposed by us [@dediu_ladd_2007;@ladd_biolinguistics_2008], where genes affect the way language changes on the *glossogenetic* timescale -- i.e across several generations.
...
In fact, this seems supported by the randomization, restricted sampling, mediation and path analyses that we conducted.
But the important take home message is that each of these methods *by itself* is not enough, especially when we try to uncover weak effects that overlap with the usual confounds (large geographic areas and language family), and thorough *multiple-method-triangulation approaches* are needed. 


**Third**, thanks to the inter-individual experimental approaches of @wong_derived_2012 and especially of @wong_aspm_lexical_2020, we can now be rather confident that the "derived" allele of *ASPM* does something to pitch perception or processing that can affect its use as a linguistic features.
Their work was testing a hypothesis generated by our exploratory work [@dediu_ladd_2007], that found a statistical association, at the population/language level, between this "derived" allele and linguistic tone.
However, as the analyses we conducted here show, this association is very weak and overlaps -- by its very nature -- the effects of family and macroarea, in such a way that a blind application of currently recommended staittsical techniques would have easily missed it!
...
Again, the analyses we conducted here suggest -- with a good dose of hindsight -- that the cross-population/corss-linguistic statistical relationship between, on the one hand, *ASPM*-D and tone, and *MCPH1*-D and tone, on the other, are of a different quality: while the first is not fully explained by the confounds, the second seems to be almost completely driven by the contrast between Africa and non-Africa (macroarea).


**Thus**, we see this as one of the most beautiful ilustrations of the joys and perils of exploratory analysis: rather than being "fishing expeditions" that no decent scientist should embark on, they are essential tools for the advancement of science.
However, there are costs and benefits to everything and the relationship between the two should be an open decision, appropariate to each field and question: too high an aversion for false positives ("costs") and there goes *ASPM*; too aversion for false negatives ("benefits") and *MCPH1* is allowed in this ratehr select club.
Luckily, in our particular fields, temporarily accepting *MCPH1* in the "causal" club together with "ASPM* did not kill any patient, blow up any multi-billion euro infrastructure or radicalise any vulnerable person, and, in fact, the additional cost of genotyping for *MCPH1*-D on top of that for *ASPM*-D in Patrick Wong's experiments were trivial.
